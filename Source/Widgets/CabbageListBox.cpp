...
void CabbageListBox::addItemsToListbox (ValueTree wData)
{
    ...
    {
        String listFile = File (CabbageWidgetData::getStringProp (wData, CabbageIdentifierIds::file)).loadFileAsString();
        StringArray lines = StringArray::fromLines (listFile);
    ...
    {
        const String currentWorkingDir = CabbageWidgetData::getStringProp (wData, CabbageIdentifierIds::workingdir);

		if (File::getCurrentWorkingDirectory().getChildFile(currentWorkingDir).exists())
			listboxDir = File::getCurrentWorkingDirectory().getChildFile(currentWorkingDir);
		else if(currentWorkingDir.isNotEmpty())
			listboxDir = File(getCsdFile()).getParentDirectory().getChildFile (currentWorkingDir);
        else
...
void CabbageListBox::valueTreePropertyChanged (ValueTree& valueTree, const Identifier& prop)
{
    if (prop == CabbageIdentifierIds::value)
    {
        if (isPresetCombo == false)
        {
            ...
            {
                int currentVal = CabbageWidgetData::getNumProp (valueTree, CabbageIdentifierIds::value);

//                if (CabbageWidgetData::getNumProp (valueTree, CabbageIdentifierIds::update) == 1)
//                    listBox.selectRow(value - 1, sendNotification);
//                else
                listBox.selectRow(currentVal - 1);//, dontSendNotification);
            }
            else
            {

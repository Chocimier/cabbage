...
{
    const String fileTmp = CabbageWidgetData::getStringProp(wData, CabbageIdentifierIds::file);
    const String relativePath = File(getCsdFile()).getParentDirectory().getChildFile(fileTmp).getFullPathName();
    if(File(relativePath).existsAsFile())
        owner->sendChannelStringDataToCsound(getChannel(), relativePath.replaceCharacters("\\", "/"));
    else
        owner->sendChannelStringDataToCsound(getChannel(), fileTmp.replaceCharacters("\\", "/"));
}
//===============================================================================

...
void CabbagePluginEditor::insertSoundfiler (ValueTree cabbageWidgetData)
{
    ...
    components.add (soundfiler = new CabbageSoundfiler (cabbageWidgetData, this, cabbageProcessor.getSampleRate()));
...
void CabbagePluginEditor::buttonClicked(Button* button)
{
	...
	else if (CabbageCheckbox* cabbageCheckBox = dynamic_cast<CabbageCheckbox*> (button))
	{
		const StringArray textItems = cabbageCheckBox->getTextArray();
		const ValueTree valueTree = CabbageWidgetData::getValueTreeForComponent(cabbageProcessor.cabbageWidgets, cabbageCheckBox->getName());
		// const int latched = CabbageWidgetData::getNumProp(valueTree, CabbageIdentifierIds::latched);

		if (textItems.size() > 0)
            cabbageCheckBox->setButtonText(textItems[buttonState == false ? 0 : 1]);
